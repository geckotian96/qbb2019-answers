...
for pos in range(len(fpkm_list)):
    ex_fpkm = 0.5126
    fpkm_list[pos] = abs(fpkm_list[pos]-0.5126)
    
    
    
fig, ax = plt.subplots()
ax.hist(fpkm_list, bins=5, color= "orange", density=True) #bin=100 can divide the range into 100 columns, default is 10. Density??

#ax.plot(x1, y1, label="Normal distribution")
ax.set_xlabel ("FPKM Residuals")
